...
def get_subscription_ids(batch: List[Dict[str, str]], sub_client) -> Tuple[
    ...
    print(f"Subscription Ids: {subscription_ids}")  # Debugging print
...
def main():
    ...
    for batch in prepare_batches(subscription_data, batch_size):
        subscription_ids, not_found_subs, invalid_subs = get_subscription_ids(
            batch, sub_client)
        print(f"Batch Result: {subscription_ids}, {not_found_subs},"
              f" {invalid_subs}")  # Debugging print

        save_not_found_subscriptions(not_found_subs)
        save_invalid_subscriptions(invalid_subs)
        for sub_id, resource_group in subscription_ids:
            print(f"Processing: {sub_id}, {resource_group}")  # Debugging print
            result = get_postgresql_servers(resource_graph_client, sub_id,
                                            resource_group)
